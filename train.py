...
def train_on_synthetic_data(input):    
    ...
    heldout_llh = eval_heldout(heldout_corpus, theta=model_theta.theta, beta=model_beta.beta)
...
list_input = pd.DataFrame({'k_gamma':[k_gamma]*20,           # Create pandas DataFrame
                      'model':[['STM','CTM']]*20,
...
Parallel(n_jobs=len(t_split))(delayed(train_on_synthetic_data)(input) for input in input_array)
