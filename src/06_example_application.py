...
K_candidates = np.array([10, 15, 20])
cores_to_use = 4
# split according to maximal cores_to_use
t = K_candidates
t_split = chunkIt(t, float(len(t) / cores_to_use))
Parallel(n_jobs=len(t_split))(
    delayed(train_on_corpus)(
        candidate, beta_train_corpus, theta_train_corpus, heldout_corpus
    )
...
result_frame = pd.DataFrame({"K": K, "heldout": np.round(values, 3)})
...
print(f"Topic {K-1} is the reference category (gamma fixed to 0)")
# %%
...
topics = [0, 1, 2, 3]
for topic in topics:
    print('Documents exhibiting topic "..."')
...
for topic in ml_topics:
    print("-" * 25)
    print(
        np.array(data[["title"]].iloc[wiki_model.find_thoughts(n=10, topics=[topic])])
    )
...
##### STATISTICS CLOUDS #######
for topic in stats_topics:
    topic_texts = np.array(
        data["text_preproc"].iloc[wiki_model.find_thoughts(n=30, topics=[topic])]
    )
    ...
    wc = WordCloud(
        ...
        random_state=1,
        background_color="white",
    ).generate(" ".join(flat_topic_texts))
    plt.imshow(wc)
    plt.axis("off")
