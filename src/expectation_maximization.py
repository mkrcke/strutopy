...
class STM:
    """ Class definition for training the model."""
    def __init__(self, settings) -> None:
        self.V = settings['dim']['V']
        self.A = settings['dim']['A']
        self.N = settings['dim']['N']
    ...
    def init_stm(self, documents, settings): 
        
        self.K = settings['dim']['K']
        
        #Random initialization, TO-DO: Improve initialization 
        mu = np.array([0]*(self.K-1))[:,None]
        sigma = np.zeros(((self.K-1),(self.K-1)))
        diag = np.diagonal(sigma, 0)
        diag.setflags(write=True)
        diag.fill(20)
        beta = random.gamma(.1,1, V*self.K).reshape(self.K,V)
        beta = (beta / beta.sum(axis=1)[:,None])
        lambd = np.zeros((N, (self.K-1)))
        
        #turn beta into a list and assign it for each aspect
        beta = np.repeat(beta,A).reshape(A,self.K,V) 
        self.init_kappa(documents, self.K, V, A, interactions=settings['kappa']['interactions'])
        
        #create model object
        model = {'mu':mu, 'sigma':sigma, 'beta': beta, 'lambda': lambd, 'kappa':self.kappa_initialized}

        return self.model

    """ Initializing Topical Content Model Parameters"""
    def init_kappa(self, documents, V, A, interactions): 
        ...
        parLength = self.K + A * aspectmod + (self.K*A)*interact
        ...
        if(aspectmod & interact == False):
            covar = {'k': np.append(np.arange(self.K), np.repeat(np.nan, A)),
                    'a': np.append(np.repeat(np.nan, self.K), np.arange(A)), 
                    'type': np.append(np.repeat(1, self.K), np.repeat(2, A))}      
        if(interact):
            covar = {'k': np.append(np.arange(self.K), np.append(np.repeat(np.nan, A), np.repeat(np.arange(self.K), A))),
                    'a': np.append(np.repeat(np.nan, self.K), np.append(np.arange(A), np.repeat(np.arange(A), self.K))), 
                    'type': np.append(np.repeat(1, self.K), np.append(np.repeat(2, A),  np.repeat(3,self.K*A)))}
        ...
        self.kappa_initialized =  self.kappa['out']
    ...
    def lhood(self, eta, mu, siginv, doc_ct, Ndoc, eta_long, beta_tuple, phi, theta, neta):
    ...
    """ Define Gradient """
    def grad(self, eta, mu, siginv, doc_ct,  Ndoc, eta_long, beta_tuple, phi, theta, neta):
    ...
    def e_step(self, documents, mu, sigma, lambd, beta):
        ...
        for i in range(N):
            ...
            result = optimize.fmin_bfgs(self.lhood,x0=eta,
    ...
    def stm_control(self, documents, vocab, settings, model=None):
        ##########
        ...
        if model == None:
            print('Call init_stm()')
            model = self.init_stm(documents, settings) #initialize
        else: 
            model = model
        ...
        while not stopits:
            ...
            ############
            # Run E-Step    
            suffstats = (self.e_step(documents, mu, sigma, lambd, beta))
            # Unpack results
            ...
            convergence = self.convergence_check(bound_ss, convergence, settings)
