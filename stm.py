    ...
    def decompose_hessian(self, hess, approx):
        ...
        try:  
            ...
            try:
                ...
                try:
                    L = scipy.linalg.cholesky(self.make_pd(hess) + 1e-5 * np.eye(hess.shape[0]))
                    print("adds a small number to the hessian")
                except: 
                    L = scipy.linalg.cholesky(approx)
        return L
...
Corpus = CorpusCreation(n_topics=num_topics, n_docs=1000, n_words=100, V=5000, treatment=False, alpha='symmetric')
